...
def init_db():
    ...
    with conn.cursor() as c:
        ...
        c.execute("UPDATE ai_config SET model_name=%s WHERE model_name != %s",
...
Always be professional, helpful, and concise. Ask one qualification question at a time naturally. When you have collected name, email, and service interest, summarize and offer to connect them with the team."""
                ...
                c.execute("""INSERT INTO ai_config (client_id, model_name, system_prompt, lead_questions_enabled, qualification_questions,
                          ...
                          (client_id, 'claude-haiku-4-5-20251001', default_prompt, default_questions,
        ...
        c.execute("SELECT id FROM clients WHERE client_slug=%s", ('cravingly',))
        if not c.fetchone():
            ...
            crav_id = c.fetchone()[0]

            c.execute("SELECT id FROM admin_users WHERE admin_id=%s", ('cravingly_admin',))
            if not c.fetchone():
                c.execute("INSERT INTO admin_users (admin_id, password_hash, role, client_id, created_at) VALUES (%s,%s,%s,%s,%s)",
                          ('cravingly_admin', hash_password('cravingly123'), 'client', crav_id, now2))

            c.execute("SELECT id FROM ai_config WHERE client_id=%s", (crav_id,))
            if not c.fetchone():
                c.execute("""INSERT INTO ai_config (client_id, model_name, system_prompt, lead_questions_enabled,
                          ...
                          (crav_id, 'claude-haiku-4-5-20251001',
...
def get_whatsapp_config_by_number(whatsapp_number):
    conn = get_conn()
    with get_db_cursor(conn) as c:
        c.execute("SELECT wc.*, c.is_active as client_active FROM whatsapp_config wc JOIN clients c ON c.id=wc.client_id WHERE wc.twilio_whatsapp_number=%s AND wc.enabled=1",
                  (whatsapp_number,))
        row = c.fetchone()
    conn.close()
    return dict(row) if row else None
